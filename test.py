...
parser.add_argument('--hgr', '-h', action='store_true', help='Use HGR scoring instead of the MSP.')
...
if 'cifar10_' in args.method_name:
    test_data = dset.CIFAR10('/nobackup/users/jk_lee/outlier/outlier-exposure-master/datasets/cifar', train=False, transform=test_transform)
    num_classes = 10
else:
    test_data = dset.CIFAR100('/nobackup/users/jk_lee/outlier/outlier-exposure-master/datasets/cifar', train=False, transform=test_transform)
    num_classes = 100
    ...
    with torch.no_grad():
        for batch_idx, (data, target) in enumerate(loader):
            ...
            if args.use_xent:
                ...
                sigmoid = nn.Sigmoid()
                smax = sigmoid(net(data, "features"))
...
ood_data = svhn.SVHN(root='/nobackup/users/jk_lee/outlier/outlier-exposure-master/datasets/svhn', split="test",
...
ood_data = dset.ImageFolder(root="/nobackup/users/jk_lee/outlier/outlier-exposure-master/datasets/places365/places365_standard/val",
                            transform=trn.Compose([trn.Resize(32), trn.CenterCrop(32),
                                                   trn.ToTensor(), trn.Normalize(mean, std)]))
ood_loader = torch.utils.data.DataLoader(ood_data, batch_size=args.test_bs, shuffle=True,
                                         num_workers=args.prefetch, pin_memory=True)
...
ood_data = lsun_loader.LSUN("/nobackup/users/jk_lee/outlier/outlier-exposure-master/datasets/lsun-master", classes='test',
                            transform=trn.Compose([trn.Resize(32), trn.CenterCrop(32),
                                                   trn.ToTensor(), trn.Normalize(mean, std)]))
ood_loader = torch.utils.data.DataLoader(ood_data, batch_size=args.test_bs, shuffle=True,
                                         num_workers=args.prefetch, pin_memory=True)
...
if 'cifar10_' in args.method_name:
    ood_data = dset.CIFAR100('/nobackup/users/jk_lee/outlier/outlier-exposure-master/datasets/cifar', train=False, transform=test_transform, download=True)
else:
    ood_data = dset.CIFAR10('/nobackup/users/jk_lee/outlier/outlier-exposure-master/datasets/cifar', train=False, transform=test_transform, download=True)
...
if 'cifar10_' in args.method_name:
    ood_data = dset.CIFAR100('/nobackup/users/jk_lee/outlier/outlier-exposure-master/datasets/cifar', train=False, transform=test_transform)
else:
    ood_data = dset.CIFAR10('/nobackup/users/jk_lee/outlier/outlier-exposure-master/datasets/cifar', train=False, transform=test_transform)
...
if 'cifar10_' in args.method_name:
    ood_data = dset.CIFAR100('/nobackup/users/jk_lee/outlier/outlier-exposure-master/datasets/cifar', train=False, transform=trn.ToTensor())
else:
    ood_data = dset.CIFAR10('/nobackup/users/jk_lee/outlier/outlier-exposure-master/datasets/cifar', train=False, transform=trn.ToTensor())
