...
class TinyImages(torch.utils.data.Dataset):
    ...
    def __init__(self, transform=None, exclude_cifar=True):
        ...
        data_file = open('/nobackup/users/jk_lee/outlier/outlier-exposure-master/datasets/80million/tiny_images.bin', "rb")
        ...
        if exclude_cifar:
            self.cifar_idxs = []
            with open('/nobackup/users/jk_lee/outlier/outlier-exposure-master/utils/80mn_cifar_idxs.txt', 'r') as idxs:
                for idx in idxs:
